...
const featureDict = {
  ...
  postHabit: "Post-Diagnosis Drug/Alcohol Use"
};
...
const distractPairs = [...Array(8).keys()].map(() => {
...
const preBasePairs = [...Array(2).keys()].map(() => {
  const a = {
    exp: rand(11, 16),
    dependents: 3
  };
  const b = {
    exp: a.exp + rand(11, 31),
    dependents: 1
  };
  return [a, b];
});

const postPairs = [...Array(10).keys()].map(() => {
  const a = {
    exp: rand(11, 16),
    dependents: 3
  };
...
export default () => {
  ...
  // track query to determine ids
  useEffect(() => {
    if (query !== undefined) {
      if (query.g !== null && query.g !== undefined) {
      ...
      if (pathname !== null || pathname !== undefined) {
        dispatch({ type: "SET_TRIAL_ID", payload: pathname.substring(1) });
      }
    }
  }, [query, pathname]);
